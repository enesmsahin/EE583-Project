...
if isTraining:
    ...
    data_path = 'D:/OKUL/6_1/EE583/PROJECT/Dataset/Train-dev/'
    base_name = 'D:/OKUL/6_1/EE583/PROJECT/Dataset/Train-dev/'
    ...
    if isHog:
        ...
        X_train = scalify.fit_transform(X_train.reshape(-1, X_train.shape[-1])).reshape(X_train.shape)
    ...
    if isHog:
        joblib.dump(sgd_clf, "svm_model.joblib")
        joblib.dump(grayify, "grayify.joblib")
        joblib.dump(hogify, "hogify.joblib")
        joblib.dump(scalify, "scalify.joblib")
    else:
        joblib.dump(sgd_clf, "svm_model_noHog.joblib")
        joblib.dump(grayify, "grayify_noHog.joblib")
        joblib.dump(scalify, "scalify_noHog.joblib")

    print("SVM Model and transformers are saved to disk!\n")
else:
    if isHog:
        sgd_clf = joblib.load("svm_model.joblib")
        grayify = joblib.load("grayify.joblib")
        hogify = joblib.load("hogify.joblib")
        scalify = joblib.load("scalify.joblib")
    else:
        sgd_clf = joblib.load("svm_model_noHog.joblib")
        grayify = joblib.load("grayify_noHog.joblib")
        scalify = joblib.load("scalify_noHog.joblib")
    print("SVM Model loaded from the disk!\n")

print("Validation Data is being loaded!")
data_path = 'D:/OKUL/6_1/EE583/PROJECT/Dataset/Validation/'
base_name = 'D:/OKUL/6_1/EE583/PROJECT/Dataset/Validation/'
include = {'Izmir', 'Metu_blue', 'Metu_red'}
data = resize_all(src=data_path, pklname=base_name, width=width, include=include)
...
if isHog:
    ...
    X_val = scalify.transform(X_val.reshape(-1, X_val.shape[-1])).reshape(X_val.shape)
